...
if [ ! -e /tmp/aws-crt-${variant}-${arch}-${version}.tar ]; then
    aws s3 cp s3://aws-crt-builder/docker/aws-crt-${variant}-${arch}-${version}.tar /tmp
...
docker exec ${container} aws s3 cp /tmp/libcrypto-${libcrypto_version}-${variant}-${arch}.tar.gz s3://aws-crt-builder/libcrypto/libcrypto-${libcrypto_version}-${variant}-${arch}.tar.gz
