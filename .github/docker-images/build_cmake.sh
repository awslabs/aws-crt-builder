...
if [ ! -e /tmp/aws-crt-${variant}-${arch}.tar.gz ]; then
    aws s3 cp s3://aws-crt-builder/releases/${version}/aws-crt-${variant}-${arch}.tar.gz /tmp
...
docker exec ${container} sh -c "cd /tmp/cmake-${CMAKE_VERSION} && make -j 4 && make install"
docker exec ${container} sh -c "tar czf /tmp/cmake-${CMAKE_VERSION}-${variant}-${arch}.tar.gz -C /usr/local share/cmake-${CMAKE_INSTALL_VERSION} bin/cmake bin/ctest bin/cpack doc/cmake-${CMAKE_INSTALL_VERSION}"
