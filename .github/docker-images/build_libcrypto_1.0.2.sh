...
if [ ! -e /tmp/aws-crt-${variant}-${arch}.tar.gz ]; then
    aws s3 cp s3://aws-crt-builder/channels/${version}/aws-crt-${variant}-${arch}.tar.gz /tmp
...
docker exec ${container} sh -c "cd /tmp/openssl && ${setarch} ./config -fPIC \
...
docker exec ${container} sh -c "cd /tmp/openssl && make && make install_sw"
